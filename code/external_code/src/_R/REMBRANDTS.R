...
outputFolder = file.path(getwd(), "output", "EISA")
dir.create(outputFolder)


########### read the exon and intron centered normalized reads, and the total read counts

exon <- read.csv(paste(path,"/vsd_normalized.exonic.all.centered.mx.txt",sep=""),sep="\t")
nSample <- ncol(exon)-1
exonRawCounts <- read.csv(paste(path,"/vsd_normalized.exonic.all.mx.txt",sep=""),sep="\t")
if( nSample != ncol(exonRawCounts)-1 )
  print("Error: unequal sample numbers")
intron <- read.csv(paste(path,"/vsd_normalized.intronic.all.centered.mx.txt",sep=""),sep="\t")
if( nSample != ncol(intron)-1 )
  print("Error: unequal sample numbers")
intronRawCounts <- read.csv(paste(path,"/vsd_normalized.intronic.all.mx.txt",sep=""),sep="\t")
...
pdf(file=paste(outputFolder,"/scatterplot.pdf",sep=""),
	width=3.00,height=3.50, units="in")
